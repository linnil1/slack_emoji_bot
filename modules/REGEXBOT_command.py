...
class REGEXBOT:
        ...
        if data != self.ori_response:
            ...
            for rep in data:
                ...
                for tri in rep['triggers']:
                    if len(tri) and tri[0] == tri[-1] == '"':
                        # no conside , in middle
                        try:
    ...
    def responseReplace(self, match, result, datadict={}):
        ...
        try:
            if match.group(1).strip() == "who":  # it can't use parse = full
                return '<@' + datadict['user'] + '|' + \
                    self.member[datadict['user']] + '>'
    ...
    def main(self, datadict):
        ...
        if any(t in self.no_response for t in
                re.findall("\w+", datadict['text'])):
            return
        ...
        with ThreadPoolExecutor(max_workers=16) as exe:
            ...
            for rep in self.response:
                for tri in rep['triggers']:
                    pool.append(exe.submit(self.triggerMatch,
                                           tri, rep, datadict['text']))
            for i in pool:
                try:
        ...
        if result:
            ...
            self.slack.api_call("chat.postMessage", **{
                ...
                "icon_url": "https://a.slack-edge.com"
                "/ae7f/plugins/slackbot/assets/service_512.png",
