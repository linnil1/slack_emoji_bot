...
class NTU118:
    ...
    def __init__(self, slack, custom):
        ...
        self.req.headers.update({
            ...
            'User-Agent': 'Mozilla/4.0 (X11; Ubuntu; Linux x86_64; rv:50.0)'
            'Gecko/20100101 Firefox/40.0',
            'Cookie': 'csrftoken=tJ6xOJKM41IdEaEcL7lge9uSAw3gRJwD'})

    def listGet(self):
        listdata = self.req.post(
            "https://118restaurant.ntu.edu.tw/restaurant/get_list",
            headers={
                'Referer': 'https://118restaurant.ntu.edu.tw/home'}
        ).json()['content']
    ...
    def mainParse(self, store):
        ...
        if len(str(store['location'])) > 5:
            gmap = "<http://maps.google.com/maps?q=loc:{},{}&z=20|".format(
                *reversed(re.findall(r"[0-9.]+", store['location'])))
    ...
    def attachParse(self, id):
        id = str(id)
        att = self.req.post(
            "https://118restaurant.ntu.edu.tw/restaurant/get_info",
            data={'data': '{"rest_id":' + id + '}'},
            headers={
                'Referer': 'https://118restaurant.ntu.edu.tw/detail/' + id}
        ).json()['content']
        ...
        if att['dishes']:
            dishes = {'title': "菜單"}
            dishes['text'] = '\n'.join(
                [i['name'] + " : " + str(i['price']) for i in att['dishes']])
            arr.append(dishes)
        ...
        if att['comments']:
            ...
            comments = {'title': "評論"}
            comments['text'] = "分數 " + \
                "{:.2}".format(allscore / len(att['comments'])) + "\n"
            comments['text'] += "\n".join(["{:★>5} -> {}".format(
                "☆" * (5 - i), score[i]) for i in range(5, 0, -1)]) + "\n"
            comments['text'] += "\n".join(
                    [i['content'] for i in att['comments'] if i['content']])

            arr.append(comments)

        return arr

    def listParse(self, arr):
        return ", ".join(arr) if len(l) else "Not found"

    def typeGet(self, type_name=""):
        type_list = self.listGet()
        if type_name:  # find restaurant of that type
            return [i['name'] for i in type_list
                    if i['food_type'] == type_name]
        # list types
        return list(set([i['food_type'] for i in type_list]))

    def foodFind(self, name):
        name_list = self.listGet()
        match = [i for i in name_list if name and i['name'].find(name) >= 0]
        if len(match) != 1:
...
`118help       ` get help"""
            ...
            data = re.findall(r"(?<=118type).*", text_input,
                              re.DOTALL)[0].strip()
            self.slack.api_call("chat.postMessage", **payload,
                                text=self.listParse(self.typeGet(data)))
            ...
            data = re.search(r"((?<=118find )|(?<=118info )).*",
                             text_input, re.DOTALL).group().strip()
