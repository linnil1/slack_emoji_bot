...
def dateTostr(year, month, date):
    ...
    url = "http://www.nongli.info/index.php?c=solar&year=" + \
        str(year) + "&month=" + str(month) + "&date=" + str(date)
...
def timeTostr(timestr):
    ...
    if timetype == '%Y':
        # only the year (7/26=Today)
        return dateTostr(thetime.tm_year, 7, 26).split(' ')[0]
    text = ""
    if '%Y' in timetype:
        text += dateTostr(thetime.tm_year, thetime.tm_mon,
                          thetime.tm_mday) + ' '
    if '%H' in timetype:
