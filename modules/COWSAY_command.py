...
# https://github.com/jeffbuttars/cowpy/tree/master/cowpy
...
def cowsay_main(parser, argtext, colorPrint):
    ...
    if args.list:
        for cowname, _ in get_cowacters(sfw=sfw, sort=True):
            answer += ("{} {}\n".format(
                cowname,
                (not_safe_for_work(cow=cowname) and ' : NSFW') or ''))
        return answer + "See more at"
        "https://github.com/jeffbuttars/cowpy#available-cowacters\n"

    if args.list_eyes:
        for k, v in get_eyes(sfw=sfw, sort=True):
            answer += ("{} : '{}'{}\n".format(
                k, v,
                (not_safe_for_work(eyes=k) and ' : NSFW') or ''))
        return answer
...
class COWSAY:
    ...
    def main(self, datadict):
        ...
        if re.search(r"^cowsay ", datadict['text']):
            text = re.search(r"(?<=cowsay ).*",
                             datadict['text'], re.DOTALL).group().strip()
            ...
            payload = {
                ...
                "text": '```' +
                        cowsay_main(self.parser, text, self.colorPrint) +
                        '```'}
