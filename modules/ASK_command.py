...
class ASK:
    ...
    def warnsGet(self, xml, podarr):
        ...
        podarr.append(self.warnGet(xml, "//languagemsg",
                                   "Language Error", "english"))
    ...
    def answerGet(self, xml, short=True, showall=False):
        if xml.attrib['success'] != "true" and \
                xml.attrib['success'] != "false":
        ...
        for pod in pods:
            ...
            if showall:
                podarr.extend([{
                    'text': " ",
                    'image_url': (p.attrib['src'])}
                    for p in pod.xpath("*/img")[1:]])
        ...
        if assum:
            podarr.append({
                "title": "Did you mean?",
                "color": "warning",
                "text": "\n".join(['[' + str(i) + '] ' + w.attrib['desc']
                                   for i, w in enumerate(assum)])})
    ...
    def main(self, datadict):
        ...
        if re.search(r"^ASK\[\d+\]", datadict['text']):
            ...
            datadict['text'] = re.sub(
                r"^ASK\[(\d+)\]", "ASK", datadict['text'])
